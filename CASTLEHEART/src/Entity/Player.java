...
public class Player extends MapObject {
	...
	// gliding
	private boolean gliding;// kind of flying
		...
		try {
			...
			for(int i = 0; i < 7; i++) {
				...
				for(int j = 0; j < numFrames[i]; j++) {
					...
					if(i != SCRATCHING) {//60 for scratching
	...
	public void draw(Graphics2D g) {
		...
		setMapPosition(); //called first in every mapobject
		...
		// draw player flinching if attacked
		if(flinching) {
			long elapsed =
				(System.nanoTime() - flinchTimer) / 1000000;
			if(elapsed / 100 % 2 == 0) {
